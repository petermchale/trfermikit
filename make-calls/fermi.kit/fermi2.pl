...
sub unitig {
	...
	my %opts = (d=>'', A=>'', t=>4, p=>'fmdef', l=>101, k=>-1, T=>61, o=>-1, m=>-1, s=>'100m');
        # Peter McHale 13 Apr 2020 and 20 Nov 2020: added 'A' and 'e' option
	getopts('d:A:t:p:k:f:r:c:l:m:s:T:2E', \%opts);
        # Peter McHale 13 Apr 2020 and 20 Nov 2020: added 'A' and 'e' option
	die (qq/
...
\n/) if (@ARGV == 0);
	...
	if (defined $opts{E}) {
		...
		push(@lines, qq/\t\/usr\/bin\/time --verbose bash -e -o pipefail -c '\$(EXE_BFC) -s \$(GENOME_SIZE) -t \$(N_THREADS) <(\$(INPUT)) <(\$(INPUT)) 2> \$@.log | gzip -1 > \$\@'/, "");
	}

	push(@lines, qq/\$(PREFIX).flt.fq.gz:\$(PREFIX).ec.fq.gz/);
	push(@lines, qq/\t\/usr\/bin\/time --verbose \$(EXE_BFC) -1s \$(GENOME_SIZE) -k \$(K_TRIM) -t \$(N_THREADS) \$< 2> \$@.log | gzip -1 > \$@/, "");

	push(@lines, qq/\$(PREFIX).flt.fmd:\$(PREFIX).flt.fq.gz/);
	push(@lines, qq/\t\/usr\/bin\/time --verbose \$(EXE_ROPEBWT2) -dNCr \$< > \$@ 2> \$@.log/, "");
	...
	push(@lines, qq/\t\$(EXE_ASSEMBLE) \$(EXE_FERMI2) \$(K_UNITIG) \$(K_MERGE) \$(N_THREADS) \$< \$@ \$(ASSEMBLY_DIAGNOSTICS)/, "");
